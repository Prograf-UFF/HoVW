...
from tree import ImageTree, ImageTreeNode

def handle_tree(tree, path):

    i = 0
    for n in tree.get_tree_data()[1:]:
        label = str(n.label)
        
        if label == None or n.mask == None:
            continue

        name = tree.name + ' ' + str(i)
        ...
        dst += '/' + re.sub('.*\/', '', name)
        n.mask.draw(output=dst, header='-'+str(i))
        i += 1
...
def clusters_len(img_trees):
    d = {}
    for it in img_trees:
        for inode in it.tree.get_tree_data()[1:]:
            l = inode.label
            if l not in d: d[l] = 1
            else: d[l] += 1
    print(d)
